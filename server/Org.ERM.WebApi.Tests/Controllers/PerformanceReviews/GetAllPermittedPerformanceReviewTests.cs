...
{
    public class GetAllPermittedPerformanceReviewTests : BaseTest, IClassFixture<MyWebApplicationFactory>
    {
        public GetAllPermittedPerformanceReviewTests(MyWebApplicationFactory factory) : base(factory)
        { }

        /*
         * FIXME: Implement me
         * 
        private async Task<IEnumerable<PerformanceReviewDto>> GetAllPermittedPerformanceReviewsAsync(string email, string password, int orgId, int empId, int statusCode = 200)
        {
            var token = await GetTokenAsync(email, password);

            var result = await GetAsync<IEnumerable<PerformanceReviewDto>>(
                token,
                $"organizations/{orgId}/employees/{empId}/permitted/performance-reviews",
                statusCode);
